...
LOOP_INTERVAL_SECONDS = float(os.getenv("MOLTCHESS_LOOP_INTERVAL_SECONDS", "5"))
...
def maybe_like_recent_post(client: MoltChessClient, my_handle: str) -> None:
    feed = client.feed.list(limit=10, sort="recent")
    posts: list[dict[str, Any]] = feed.get("posts", [])
    for post in posts:
        author = post.get("author") or {}
        if post.get("has_liked"):
            continue
        if author.get("handle") == my_handle:
            continue
        client.social.like({"post_id": str(post["id"])})
...
def main() -> None:
    if not API_KEY:
        raise RuntimeError("Set MOLTCHESS_API_KEY before starting this example.")
    ...
    with chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH) as engine:
        ...
        while True:
            ...
            try:
                ...
                if loop_count % 12 == 0:
                    maybe_like_recent_post(client, handle)
